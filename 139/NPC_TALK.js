    ...
    switch (Byt_msgType) {
        ...
        case 0x6:
            ...
            if (type == 0x25) {
                ...
                //旧选择框 type2==0   不带停止按钮 NPC右边
                mplew.LogAppend("cm.sendSimpleforce(\"" + talk + "\")");
            } else if (type == 0x38) {
                //新选择框 type2==1   不带停止按钮
                mplew.LogAppend("cm.sendTNewSimpleforce(\"" + talk + "\")");
            } else if (type == 0) {
        ...
        case 0x10:
            if (type == 0) {
                ...
                //type==0 tye2==0 旧对话框 接受拒绝 不带停止 玩家头像
                mplew.LogAppend("cm.sendTAcceptDeclineforce(\"" + talk + "\")");
            } else if (type == 1) {
                //type==0 tye2==0 旧对话框 接受拒绝 不带停止
                mplew.LogAppend("cm.sendAcceptDeclineforce(\"" + talk + "\")");
            } else if (type == 0x20) {
                //type==0 tye2==1 新對話框 接受拒绝 带停止按钮
                mplew.LogAppend("cm.sendNewAcceptDecline(\"" + talk + "\")");
            }
            break;
        case 0x13:
            mplew.LogAppend("次元之镜(\"" + talk + "\")");
            break;
        case 0x1A: //旧对话框 带 大 NPC 图片
            switch (zeroPad(typea, 2) + zeroPad(typea2, 2)) {
                case "0000":
                    //旧对话框 带 大 NPC 图片 确认按钮带 停止
                    mplew.LogAppend("cm.sendBOk(\"" + talk + "\"" + ((type & 4) ? "," + Int_npcID : "") + ")");
                    break;
                case "0001":
                    //旧对话框 带 大 NPC 图片 下一步 没有停止按钮
                    mplew.LogAppend("cm.sendBNextforce(\"" + talk + "\")");
                    break;
                case "0101":
                    //上一步 下一步 没有停止按钮 type ==5  大图右边 
                    mplew.LogAppend("cm.sendBNextPrevforce(\"" + talk + "\"" + ((type & 4) ? "," + Int_npcID : "") + ")");
                    break;
                case "0100":
                    //上一步 确认 没有停止按钮  type ==5  大图右边 
                    mplew.LogAppend("cm.sendBPrev(\"" + talk + "\"" + ((type & 4) ? "," + Int_npcID : "") + ")");
                    break;
                default:
            ...
            switch (zeroPad(typea, 2) + zeroPad(typea2, 2)) {
                case "0100":
                    if (type == 0 || type == 4) {
                        ...
                        mplew.LogAppend("cm.sendTOkNew(\"" + talk + "\"" + ((type & 4) ? "," + Int_npcID : "") + ")");
                    }
                    break;
                case "0101":
                    if (type == 0x4) {
