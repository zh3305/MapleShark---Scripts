...
if (CharInfo) {
    ...
    var mask = mplew.writeLong("Flag", -1);
    mplew.write("Unknown");
    mplew.StartNode("Seeds");
    for (i = 1; i <= 3; i++) {
        mplew.writeInt("timeGetTime+", -6);
    }
    mplew.EndNode(false);
    ...
    if ((mask & 0x02) != 0) {
        ...
        // CInPacket::DecodeBuffer(ipacket, v13 + 2462, 0xC);
        mplew.writeInt("Character Id", playerid);
        mplew.writeInt("Unknown", 0);
        mplew.writeInt("Unknown", 0);
    }
    mplew.EndNode(false);
    mplew.StartNode("flag & 8|| mask & 0x2000000");
    if ((mask & 0x08 || mask & 0x2000000) != 0) {
    ...
    mplew.EndNode(false);
    mplew.StartNode("flag & 0x8000 物品信息");
    if ((mask & 0x8000) != 0) {
        mplew.StartNode("Unknown");
        var unksize = mplew.writeInt("unksize", 0);
        for (i = 0; i < unksize; i++) {
    ...
    if ((mask & 0x100) != 0) {
        ...
        mplew.EndNode(false);
        mplew.StartNode("可接受传授的技能列表 ");
        count = mplew.writeInt("可接受传授的技能列表"); //可接受传授的技能列表
    ...
    if ((mask & 0x1000) != 0) {
        ...
        for (i = 1; i <= 28; i++) {
            mplew.writeInt("Teleport Rock " + i + " Map Id", 999999999);
        }
        mplew.EndNode(false);
    }
    mplew.EndNode(false);
    ...
    var v251 = mplew.write("UNK 139 NEW");
    if (v251) {
        if (mask & 0x100) {
            for (k = mplew.writeInt("k"); k > 0; --k) {
                v252 = mplew.writeInt("v252");
                mplew.writeMapleAsciiString("");
                // v356 = 27;
                // sub_781CE0( /* v252, &v321 */ );
                // v356 = 6;
                // if (v321)
                //     sub_484D80( /* v321 - 12 */ );
            }
    ...
    if ((mask & 0x20000) != 0) { //RunnerGameRecord::Decode
        ...
        mplew.writeLong("tLastPlayed getTime(-2)", "94354848000000000");
    ...
    mplew.EndNode(false);
    mplew.StartNode("flag & 0x200000  139new")
...
function addCharStats() {
    ...
    playerid = mplew.writeInt("Id");
    mplew.writeInt("Id", playerid);
    mplew.writeInt("Unknown");
    ...
    mplew.writeLong("-2L Time", "131263822532630000");
    ...
    mplew.write("", 0);
    mplew.writeLong("- Time", "94354848000000000");
...
function addInventoryInfo() {
    ...
    mplew.write("其他栏数量");
    mplew.write("现金栏数量");
    mplew.writeLong("Default Expiration Time", "94354848000000000");
    ...
    DecodeEquipmentInventory("139 new");
    // DecodeEquipmentInventory("1392 new");
    // DecodeEquipmentInventory("1393 new");
    // DecodeEquipmentInventory("1393 new");
...
function addRedLeafInfo() {
    ...
    show = [9410165, 9410166, 9410167, 9410168, 9410198]
    mplew.writeInt("Unknown", 0);
    for (i = 1; i <= count; i++) { //Packet_ReadBuffer(a2, (v2 + 4), 0x28u);
        mplew.StartNode("addRedLeafInfo " + i);
        mplew.writeInt("9410165-9410168", show[i - 1]);
